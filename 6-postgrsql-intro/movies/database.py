...
#  a linking table making a m-to-m rel between movies and users
...
# table.* selects only rows from that table
...
# add an index to make searching by release date quicker
CREATE_RELEASE_INDEX = "CREATE UNIQUE INDEX IF NOT EXISTS idx_movies_release ON movies(release_timestamp);"


# create the db connection using the environment variable
connection = psycopg2.connect(os.environ["DATABASE_URL"])

def create_tables():
    with connection:
        # use with to create cursor which releases resources when it is done
